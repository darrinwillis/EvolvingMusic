...
public class Fitness {
	public static class Report {
		public double size;
		public double keyScore;
		public double rhythymScore;
		public double chordScore;
		public double leapScore;
		public double progressionScore;
	...
	protected static double calcFitness(MusicTree mt)
	{
		...
		// Progression score
		score -= Math.min(Math.exp(Math.abs(getLength(mt) - idealLength) / 2),
				score);
		return score;
	}
	...
	public static Report getReport(MusicTree mt)
	{
		...
		fitRep.lengthScore = Math
				.exp(Math.abs(getLength(mt) - idealLength) / 2);
		return fitRep;
	}
	...
	private static double evaluateKey(MusicTree mt)
	{
		...
		long numMajors = events.stream()
				.filter((me) -> me.note.pitch.isMajor()).count();
		long numPents = events.stream()
				.filter((me) -> me.note.pitch.isPentatonic()).count();

		// keynum is the number of notes in the key which is closest to this
		// collection of notes
	...
	private static double evaluateRhythm(MusicTree mt)
	{
		...
		Map<Integer, Integer> eventsPerBeat = events.stream().collect(
				Collectors.groupingBy(me -> me.time % 16,
						Collectors.summingInt(me -> 1)));
		assert eventsPerBeat.size() <= 16;
		assert eventsPerBeat.values().stream().mapToInt(Integer::intValue)
				.sum() == numEvents;
		double numOnBeats = 2 * eventsPerBeat.getOrDefault(0, 0)
				+ eventsPerBeat.getOrDefault(4, 0) + 1.5
				* eventsPerBeat.getOrDefault(8, 0)
	...
	private static double evaluateChords(
			Map<Integer, List<MusicEvent>> timedEvents)
	{
		int numChords = 0;
		for (List<MusicEvent> events : timedEvents.values())
		{
			Set<Pitch> pitches = events.stream().map(me -> me.note.pitch)
	...
	private static double evaluateProgression(
			Map<Integer, List<MusicEvent>> events)
	{
		List<Pitch.Chord> chords = events
				.values()
				.stream()
				.map(l -> l.stream().map(me -> me.note.pitch)
	...
	{
		List<MusicEvent> events = mt.getRender();
		Map<Integer, List<MusicEvent>> timedEvents = events.stream().collect(
				Collectors.groupingBy(me -> me.time));
		return timedEvents;
	}
